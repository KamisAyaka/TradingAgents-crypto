...
def open_binance_position_usdt(
    ...
    try:
        ...
        return str(exc)
    return (
        f"下单成功：{resp.get('symbol')} {resp.get('side')} {resp.get('executedQty')} "
        f"张（约等于 {notional_usdt} USDT 名义），订单 ID {resp.get('orderId')}。"
...
def close_binance_position(
    ...
    try:
        ...
        return str(exc)
    return (
        f"已平仓 {resp.get('symbol')} {resp.get('executedQty')} 张，"
...
def set_binance_take_profit_stop_loss(
    ...
    if "stop_loss" in result:
        ...
        parts.append(
            f"止损单已创建（订单 ID {sl_resp.get('orderId')}，触发价 {sl_resp.get('stopPrice')}）。"
        )
    if "take_profit" in result:
        tp_resp = result["take_profit"]
        parts.append(
            f"止盈单已创建（订单 ID {tp_resp.get('orderId')}，触发价 {tp_resp.get('stopPrice')}）。"
        )
