...
def create_crypto_newsflash_analyst(llm):
    ...
    def crypto_newsflash_node(state):
        ...
        newsflash_text = get_crypto_newsflash.invoke({"limit": 20, "lookback_hours": 24})
        ...
        conversation = list(state["messages"]) + [
            HumanMessage(content=f"【Odaily 快讯数据】\n{newsflash_text}")
        ]

        result = chain.invoke(conversation)
        report = result.content or "【错误】快讯分析生成失败。"
